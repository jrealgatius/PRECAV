...
params:
  evaluar: TRUE 
  fitxer_entrada: "BD_PRECAV_test5.rds" 
...
```{r inicialitzacio, include=T}

if (params$mostra) dir_preparades<-"dades/preparades/test" else dir_preparades<-"dades/preparades"


fitxer_input<-here::here(dir_preparades,"BD_PRECAV.rds")
# fitxer_input<-here::here(dir_preparades,params$fitxer_entrada)


dades_flow_input<-here::here(dir_preparades,"dades_flow.rds")
...
# 1.2. Generació de conjunt de dades imputades

```{r imputacio, eval=F}

# Fer IMPUTACIO I GUARDAR LONG + DATASET INICIAL  ----------------------

# Mice metode
library(mice)
# 1. Selecciono data_set de variables que vull utilitzar per la imputació   -------
variables_a_imputar<-extreure.variables("imp",taulavariables = conductor_variables)
...
num_imputacions<-1 # Normalment >=10
...
dades<-dades %>% mutate(DG.E11=ifelse(!is.na(DG.DM_O) & is.na(DG.E11),DG.DM_O,DG.E11))

# Calcul de temps DM2
dades<-dades %>% 
  mutate(Duracion_DM2 = as.period(interval(start = ymd(DG.E11), end = ymd(dtindex))) %>% time_length("year")) 

# Calcul de temps DM1
dades<-dades %>% 
  mutate(Duracion_DM1 = as.period(interval(start = ymd(DG.E10), end = ymd(dtindex))) %>% time_length("year")) 
...
##  Calcular edat 
dades<-dades %>% 
  mutate(edat = lubridate::year(as.period(interval(start = ymd(dnaix), end = dtindex))))
...
dades_regicor<-selectorvariables("regicor",taulavariables=conductor_variables,dt=dades) %>% 
  mutate(age=edat,
         diabetes=if_else(ANT1_3.E11==1 | ANT1_3.E10==1,1,0),
...
dades<-dades %>% mutate(
  score2=
    score2(sex=sexe,age=edat,
           smoke =tab.valor,
           sbp=PAS.valor13a,
           dm=as.numeric(ANT1_3.E11==1 | ANT1_3.E10==1),
           coltot=COLTOT.valor13a,
           colhdl=COLHDL.valor13a,
           risk_region = "Low")) 
...
# Generar variable event primari grup a risk: (seleccionar grups a risk només amb event primari)
...
** Ojo que només s'excluen FA / i per edat i grups corresponents sense control
...
```{r filtratje}
# Dades flow-chart
# Aplicar filtre inicial de grup
dades<-dades %>% filter(eval(parse(text=params$filtre)))
...
flow_chart<-criteris_exclusio_diagrama(dt=dades,taulavariables = conductor_variables,
                                       criteris = "c_exclusio1",etiquetes = "exc_lab",
                                       grups = "event",ordre="exc_ordre",sequencial=T,
                                       pob_lab=c("Pob aparellada nascuda: 1935-1981","Pob final"))
...
NControls_inicial_final<-dades_flow %>% filter(grup=="Control" & (etiqueta=="Inicial" | etiqueta=="Post_Match")) %>% pull(N) 
NCas_inicial_final<-dades_flow %>% filter(grup=="Cas" & (etiqueta=="Inicial" | etiqueta=="Post_Match")) %>% pull(N) 
...
# 4. Etiquetació / factorització / relevels
...
vars_factor<-read_conductor(conductor_variables) %>% filter(factor.YESNO>0) %>% pull(camp)

dades<-dades %>% mutate_at(vars_factor,~factor(.,levels=c(0,1), labels=c("No","Yes"))) 
...
dades<-dades %>% mutate(CKDEPI.valor13a.cat4=factor(CKDEPI.valor13a.cat4,levels = rev(levels(CKDEPI.valor13a.cat4))))

# Relevel MEDEA U1 Referencia 
dades<-dades %>% mutate(qmedea=factor(qmedea,levels = rev(levels(qmedea))))
dades<-dades %>% mutate(qmedea=relevel(qmedea,"U1"))
...
M2.reg.vars.ls<-extreure_model_logistic(x="regicor_vars2",y="event",taulavariables=conductor_variables,dades=dades,valor_outcome=cat_event,conditional = T,strata = "caseid")
# Numero de efectius amb variables regicor
n_regicor_vars<-na.omit(selectorvariables("regicor_vars2",taulavariables = conductor_variables,dades)) %>% count()
...
# Numero de efectius amb variables regicor_plus
n_regicor_plus<-na.omit(selectorvariables("regicor_plus",taulavariables = conductor_variables,dades)) %>% count()
...
M2.reg.vars.G1.ls<-extreure_model_logistic(x="regicor_vars2",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
...
>- Evaluar Regicor Grup d'edat Grans (G2)
...
M2.reg.vars.G2.ls<-extreure_model_logistic(x="regicor_vars2",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
...
>- Evaluar Regicor Grup dones Joves (G3)

```{r regicor_g3, eval=T, include=F}
dades_temp<-dades %>% filter(edat_grup=="Young:M<55/F<65" & sexe=="D")

M1.reg.alon.G3.ls<-extreure_model_logistic(x="regicor_alone",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
M2.reg.vars.G3.ls<-extreure_model_logistic(x="regicor_vars2",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
M3.reg.plus.G3.ls<-extreure_model_logistic(x="regicor_plus",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")

```

>- Evaluar Regicor Grup dones Granss (G4)

```{r regicor_g4, include=F, eval=T}

dades_temp<-dades %>% filter(edat_grup=="Old:M>=55/F>=65" & sexe=="D")
M1.reg.alon.G4.ls<-extreure_model_logistic(x="regicor_alone",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
M2.reg.vars.G4.ls<-extreure_model_logistic(x="regicor_vars2",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
M3.reg.plus.G4.ls<-extreure_model_logistic(x="regicor_plus",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")


```

>- Evaluar Regicor Grup homes Joves (G5)

```{r regicor_dones_joves, eval=T, include=F}
dades_temp<-dades %>% filter(edat_grup=="Young:M<55/F<65" & sexe=="H")

M1.reg.alon.G5.ls<-extreure_model_logistic(x="regicor_alone",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
M2.reg.vars.G5.ls<-extreure_model_logistic(x="regicor_vars2",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
M3.reg.plus.G5.ls<-extreure_model_logistic(x="regicor_plus",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")

```

>- Evaluar Regicor Grup homes Granss (G6)

```{r regicor_g6, include=F, eval=T}

dades_temp<-dades %>% filter(edat_grup=="Old:M>=55/F>=65" & sexe=="H")
M1.reg.alon.G6.ls<-extreure_model_logistic(x="regicor_alone",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
M2.reg.vars.G6.ls<-extreure_model_logistic(x="regicor_vars2",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
M3.reg.plus.G6.ls<-extreure_model_logistic(x="regicor_plus",y="event",taulavariables=conductor_variables,dades=dades_temp,valor_outcome=cat_event,conditional = T,strata = "caseid")
...
# Netejo ambient
gc()
rm(list=c("dadesevents","dades_temp","flow_chart_antic","flow_chart_antic2","temp_dt","params"))
rm(params)

save.image(file=here::here(dir_Rdata,"output_PRECAV.Rdata"))
# saveRDS(dades,here::here(dir_Rdata,"dades.Rds"))
