...
strataTable<-function (x, strata, strata.names = NULL, max.nlevels = 5) 
{
  if (missing(strata)) 
  ...
  x.list <- lapply(levels(strata.var), function(i) {
    subset.i <- paste0("as.factor(", strata, ")=='", 
                       i, "'", global.subset)
    cg.i <- eval(parse(text = paste0("update(cg, subset=", 
                                     subset.i, ", simplify=FALSE)")))
