...
LLEGIR.PACIENTS<-function(n=Nmostra) {
  readRDS("ECV_CAT_entregable_pacients_20181128_152759.rds") %>% as_tibble() %>% hean(n)}
...
PACIENTS<-LLEGIR.PACIENTS(Inf)
PROBLEMES<-LLEGIR.PROBLEMES(Inf)
CMBDH<-LLEGIR.CMBDH(Inf)
CMBDH.padris<-LLEGIR.padris(Inf)
CMBDH_PROC<-LLEGIR.PROC(Inf)
...
VISITES=LLEGIR.VISITES(Nmostra)
...
FX.PRESCRITS<-LLEGIR.FX.PRESCRITS(Nmostra)
