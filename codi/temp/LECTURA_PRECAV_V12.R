...
fitxer_conductor_cataleg<-"cataleg_definitiu.xls"
...
dates_casos1<-PROBLEMES_total %>% 
  agregar_problemes_agr(bd.dindex="20161231",agregador =AGR_CAS ,dt.agregadors=CATALEG,finestra.dies=c(-Inf,0),prefix="",camp_agregador="AGR_PR") %>%  # agrego problemes de salut
  select_("idp","dtevent"=AGR_CAS,"codindex"="cod")

casos_AGR_PR<-dates_casos1 %>% group_by(codindex) %>% count()

write.csv2(casos_AGR_PR,"CASOS_ECV_PRIMARI.txt")

AGR_CAS<-"ECV_TER"
dates_casos2<-PROBLEMES_total %>% 
...
casos_AGR_TR<-dates_casos2 %>% group_by(codindex) %>% count()
write.csv2(casos_AGR_TR,"CASOS_ECV_TR.txt")
...
events_dt<-PACIENTS %>% filter(event==1)


bdades_events<-events_dt %>% group_by(codindex) %>% count()

write.csv2(bdades_events,file="dades_events")
...
CLINIQUES<-Nmostra %>% LLEGIR.CLINIQUES %>% select(idp,cod,val,dat) 
cliniques_bd<-agregar_analitiques(dt=CLINIQUES,bd.dindex =bdades_index,finestra.dies=c(-365,0))
rm(CLINIQUES)

#     6.5. Agregar TABAC ---------------------------
TABAC<-Nmostra %>% LLEGIR.TABAC %>% mutate(agr="tab")
...
BDTOTAL<-bdades_index %>% 
  left_join(pp, by="idp") %>% select(-dataindex) %>% 
