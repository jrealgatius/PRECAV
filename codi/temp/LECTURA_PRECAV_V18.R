...
fitxer_conductor_cataleg<-"cataleg_definitiu.9.5.19_def.xls"


# fitxer conductor variables
fitxer_conductor_variables<-"variables_precav.9.5.19.def.xls"
...
problemes_ANT1_3a<-agregar_problemes(dt=PROBLEMES_total,bd.dindex =bdades_index,dt.agregadors=CATALEG,finestra.dies=c(-1095,-366),camp_agregador="agr",prefix = "ANT1_3.")
...
gc()
variables_bd1_3a<-agregar_analitiques(dt=VARIABLES,bd.dindex =bdades_index,finestra.dies=c(-1095,-366),sufix = c(".valor13a", ".dies13a"))
rm(VARIABLES)
...
gc()
cliniques_bd1_3a<-agregar_analitiques(dt=CLINIQUES,bd.dindex =bdades_index,finestra.dies=c(-1095,-366),sufix = c(".valor13a", ".dies13a"))
rm(CLINIQUES)
gc()
...
tabac_bd1_3a<-agregar_analitiques(dt=TABAC,bd.dindex =bdades_index,finestra.dies=c(-1095,-366),sufix=c(".valor13a", ".dies13a"))
rm(TABAC)
gc()
#     6.7. Agregar Farmacs Facturació ---------------------------
...
farmacs_dispensats_1_3a<-agregar_facturacio(dt=FX.FACTURATS,prefix = "FD1_3a.",bd.dindex =bdades_index,finestra.dies=c(-1095,-366),dt.agregadors=CATALEG)
rm(FX.FACTURATS)
gc()
#     6.8. Agregar Farmacs Prescrits ----------------------
FX.PRESCRITS<-Nmostra %>% LLEGIR.FX.PRESCRITS
farmacs_prescrits<-agregar_prescripcions(dt=FX.PRESCRITS,bd.dindex=bdades_index,dt.agregadors=CATALEG,prefix="FP.",finestra.dies = c(-365,-45))
farmacs_prescrits1_3a<-agregar_prescripcions(dt=FX.PRESCRITS,bd.dindex=bdades_index,dt.agregadors=CATALEG,prefix="FP1_3a.",finestra.dies=c(-1095,-366))
rm(FX.PRESCRITS)
gc()
...
write.csv2(variables_generades,"variables_precav.txt")

# 10. Salvar variables noves en conductor
...
write.csv2(variables_noves,"variables_noves_precav.txt")
