...
PACIENTS <- PACIENTS %>% 
  ...
  mutate (dtsortida=as.Date(as.character(dtsortida),format="%Y%m%d") %>% as.numeric()) 

PACIENTS %>% filter(event==1) %>% select(dtsortida)
...
dt %>% filter(event==0) %>% select(dtsortida) 
...
problemes_ANT1_3a<-agregar_problemes(dt=PROBLEMES_total,bd.dindex =bdades_index,dt.agregadors=CATALEG,finestra.dies=c(-1095,-45),camp_agregador="agr",prefix = "ANT1_3.")
...
tabac_bd1_3a<-agregar_analitiques(dt=TABAC,bd.dindex =bdades_index,finestra.dies=c(-1095,-45),sufix=c(".valor13a", ".dies13a"))
rm(TABAC)
gc()
