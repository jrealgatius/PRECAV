...
bdades.dades.semirandom<-bdades.dades.semirandom %>% 
  mutate(dtindex.random =ymd(dtindex.semirandom), # Converteixo en format data 
...
###                                 AGREGACIÓ DE PROBLEMES DE SALUT                         #
...
######          dtindex_case = Data event  / dtindex_control=dtindex.random
dt<-dt %>% mutate(dtindex_case=ifelse
                  (event==1, as.integer(lubridate::ymd(dtsortida)),
                           as.integer(dtindex.random))
                  )

dt<-dt %>% mutate(dtindex_control=ifelse
  (control==1,as.integer(dtindex.random),dtindex_case),
  dtindex_control=ifelse(is.na(dtindex_control),dtindex_case,dtindex_control)
  )
...
dt<-dt %>% mutate(dtindex_control=ifelse
                  (control==1,as.integer(dtindex.random),NA)
                  )

#### Elimino aquells que no tinc data index o (data censura)

dt<-dt %>% filter(dtindex_case | dtindex_control) 


##### Genero any_calendari

dt<-dt %>% mutate(any_calendari=ifelse
                  (event==1, as.integer(lubridate::year(lubridate::ymd(dtsortida))),
                    as.integer(lubridate::year(dtindex.random)))
                  )

####  Genero edat en data index o data censura

dt<-dt %>% mutate (edat=ifelse(event==1,as.integer((ymd(dtsortida)-ymd(dnaix))/365.25),
                                        as.integer((dtindex.random-ymd(dnaix))/365.25)))
...
### Encara no estic segur si he de posar la data index per aparellar 

# dt <- dt %>% mutate (any_index=ifelse(event==1,year(dtindex_case),year(dtindex.random)))
              
                
#################################     FUNCIÓ riskSetMatch           ################ 

#### provo d'assignar 31/12/2016 data index controls 

dt<-dt %>% mutate (dtindex_control=as.integer(lubridate::ymd(20161231)))
...
llistaPS=c("sexe","any_naix","any_calendari")
...
pp<-riskSetMatch(ptid="idp"                                # Unique patient identifier
                         ...
                         ,Ncontrols=50                  # number of controls to provide
...
llistaPS=c("sexe","edat","any_calendari")
