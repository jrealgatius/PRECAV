...
pp<-riskSetMatch(ptid="idp"                                # Unique patient identifier
                 ...
                 ,Ncontrols=10                  # number of controls to provide
...
######################      ARA TOCA AGREGAR EN DATA INDEX TOT    #######################
pp<-pp %>% mutate (
...
analitiques_bd<-agregar_analitiques(dt=ANALITIQUES_total,bd.dindex =bdades_index,finestra.dies=c(-Inf,0))
...
tabac_bd<-agregar_tabac(dt=TABAC,bd.dindex =bdades_index,finestra.dies=c(-Inf,0))

##########    BDINDEX + PACIENTS + PROBLEMES + VARIABLES + VISITES + TABAC 

# bdades_index + pp + problemes_bd + analitiques_bd + visites_bd + tabac_bd 
