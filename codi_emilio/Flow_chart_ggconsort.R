...
generar_dummies_exclusions<-function(dt=iris, criteris=c("Sepal.Length < 5","Species=='setosa'")) {
  ...
  dt_dumies<-
    map2_dfc(criteris,cols_dummies,
       ~transmute(dt,!!sym(.y):=eval(parse(text=.x)))
    ) %>% mutate_all(as.numeric)
  dt %>% bind_cols(dt_dumies) 
...
Flow_chart_Consort<-function(dt=trial_data,
  ...
  if (is.na(grups)) 
    {dt<-dt %>% dplyr::select(exclusions) %>% dplyr::mutate(grup="Overall",idp = row_number())
    grup<-"Overall"} else 
      {dt<-dt %>% dplyr::select(exclusions,grup=grups) %>% dplyr::mutate(idp = dplyr::row_number())}
  
  # capturar etiquetes
  if (is.null(lab_exclusions)) labels_exclusions<-exclusions else labels_exclusions<-lab_exclusions
