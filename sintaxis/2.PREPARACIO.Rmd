...
# regicor_mis 
dades<-dades %>% mutate(regicor_mis=ifelse(is.na(regicor),"Missing","Any value"))
# Factoritzo
...
T1.4.EVENTS<-descrTable(formula,data=dadesevents,show.p.overall = F,hide.no = "No",show.all = T)

formula<-formula_compare(x="events_ter",y="edat_pre",taulavariables = conductor_variables) 
T1.5.EVENTS.2<-descrTable(formula,data=dadesevents,show.p.overall = F,hide.no = "No",show.all = T)

formula<-formula_compare(x="events_imp",y="edat_pre",taulavariables = conductor_variables) 
T1.6.EVENTS<-descrTable(formula,data=dadesevents,show.p.overall = F,hide.no = "No",show.all = T)
...
# OR i sig Global
M1.reg.alon_conti.ls$taula_OR %>% kable(digits = 3,caption = "Global") %>% kableExtra::kable_styling()
M1.reg.alon.ls$taula_OR %>% kable(digits = 3) %>% kableExtra::kable_styling()
M1.reg.mis.ls$taula_OR %>% kable(digits = 3) %>% kableExtra::kable_styling()
...
Taula1.D<-descrTable(formula,data=dades_temp,show.p.overall = F,hide.no = "No",show.n = T,show.ratio = F)
