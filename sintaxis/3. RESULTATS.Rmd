...
&check; Eliminados casos no matcheados
&check; Arreglado algoritmo usando RisksetMatch() busqueda de controles última versión paquete heaven
&check; Generado algoritmo de control de versiones de paquete heaven de R (previsión de formateo IT)
&check; Aplicado criterio de exclusión DM1 + FA 
&check; Rehecho flow-chart segun nuevos criterios de inclusión
&check; Actualización tablas con nueva base de datos
&check; Nuevas tablas segun objetivos estratificadas por edad y sexo

## Realizado:

&check; Encontrado fallo del bajo registro de los Anti HTA 
&check; Reanalizado con datos de facturaciones i prescripciones anti-HTA
...
>> Operativo: >> Descriptiva y estimación de associación estadística (OR) de punutación del regicor y ser caso en població joven
...
M1.reg.alon_conti.ls.H$taula_OR %>% kable(digits = 3,caption = "Varones") %>% kableExtra::kable_styling()
M1.reg.alon.ls.H$taula_OR %>% kable(digits = 3) %>% kableExtra::kable_styling()
M1.reg.mis.ls.H$taula_OR %>% kable(digits = 3) %>% kableExtra::kable_styling()
M1.reg.alon.ls.H$forest_plot

# OR i sig estrat: Dones
M1.reg.alon_conti.ls.D$taula_OR %>% kable(digits = 3,caption = "Mujeres") %>% kableExtra::kable_styling()
M1.reg.alon.ls.D$taula_OR %>% kable(digits = 3) %>% kableExtra::kable_styling()
M1.reg.mis.ls.D$taula_OR %>% kable(digits = 3) %>% kableExtra::kable_styling()
...
# 2.Análisis exploratorio actualizado
...
kable(M2.reg.vars.ls$taula_OR,digits = 3) %>% kableExtra::kable_styling()
M2.reg.vars.ls$forest_plot
M2.reg.vars.ls$ggplot_ROC
M2.reg.vars.ls$auc_ci
